...
When building projects, follow this precedence order:

1. **`project_design.md`** (if present) - Project-specific design decisions that override everything else
2. **`design.md`** (if present) - General design system and styling guidelines
3. **`claude.md`** (this file) - Technical requirements and setup instructions

**Always check for these files before starting work.** If `design.md` or `project_design.md` exist in the project, read them first to understand the design requirements.
...
- Colors, spacing, and breakpoints live in `tailwind.config.ts`.
...
### Rule of Thumb
- **Local, one-off styles** → keep them inline in .tsx files using Tailwind utilities
...
1. **Check for design files first** - Look for `project_design.md` or `design.md`
2. **Follow the hierarchy** - Project-specific overrides general, which overrides defaults
3. **Stay consistent** - Use the design system's colors, spacing, and patterns
4. **Use Tailwind utilities** - Avoid custom CSS unless specified in design files
5. **Be responsive** - Follow mobile-first principles from the design system
6. **Use subagents for validation** - Invoke `@design-validator` or `@design-auditor` at key checkpoints